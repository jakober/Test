...
class Resizer {
    ...
    function __construct($file) {
        ...
        if (!$this->image) {
            throw new Exception('File not recognized. Possibly because the path is wrong. Keep in mind, paths are relative to the main index.php file.');
        }
    ...
    public function resize($new_width, $new_height, $option = 'auto') {
        ...
        $optimal_width = floor($option_array['optimal_width']);
        $optimal_height = floor($option_array['optimal_height']);

        $this->thumb_width = $optimal_width;
        $this->thumb_height = $optimal_height;
        ...
        $background_colour = imagecolorallocate(
                $image_background, array_get($this->config, 'background_color.r'), array_get($this->config, 'background_color.g'), array_get($this->config, 'background_color.b')
        );
    ...
    private function crop($optimal_width, $optimal_height, $new_width, $new_height) {
        ...
        imagefilledrectangle($this->image_resized, 0, 0, $new_width, $new_height, imagecolorallocatealpha($this->image_resized, 255, 255, 255, 127)
        );
